...
const SignUp = () => {
    ...
    useEffect(() => {
        // Resetear los campos cuando se monta el componente
        setEmail('');
        setPassword('');
        setUserName('');
        setPassword2('');
    ...
    const onClickRegister = (event) => {
        ...
        if(!validPassword.test(password)) {
            list.push("La contraseña debe tener al menos 6 caracteres alfanuméricos");
        }
                ...
                <>
                    <div className="flex-auto p-2 grid gap-6 mb-6 md:grid-cols-2">
                        <div>
                            ...
                            <input type="text" value={userName} onChange={onSetUserName} id="first_name" className="bg-yellow-50 border border-red-300 text-gray-900 text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-red-500 dark:focus:border-red-500" placeholder="Juan" required />
                        </div>
                        <div className="mb-6">
                            <label htmlFor="email" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Correo Electrónico</label>
                            <input type="email" value={emailInput} onChange={onSetEmail} id="email" className="bg-yellow-50 border border-red-300 text-gray-900 text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-red-500 dark:focus:border-red-500" placeholder="juan.rodriguez@company.com" required />
                        </div>
                        <div className="mb-6">
                            <label htmlFor="password" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Contraseña</label>
                            <input type="password" value={password} onChange={onSetPassword} id="password" className="bg-yellow-50 border border-red-300 text-gray-900 text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-red-500 dark:focus:border-red-500" placeholder="•••••••••" required />
                        </div>
                        <div className="mb-6">
                    ...
                    <button type="submit" onClick={onClickRegister} className="text-white bg-yellow-700 hover:bg-yellow-900 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm w-full sm:w-auto px-4 py-2.5 text-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800">Ingresar</button>
                </>
            ) : (
                <>
